...
datapath = "dataset.patients.json"
...
def main():
    ...
    for epoch in range(n_epochs):
        for (i, j) in list(zip(i_usr_pred, i_item_pred)):
            ...
            # pred += np.dot(pu[user,:],qi[item,:])

            # bu[user] += lr * (err - reg * bu[user])
            # bi[item] += lr * (err - reg * bi[item])

            # for factor in range(n_factors):
            #     pu_pred = pu[user, factor]
            #     qi_pred = qi[item, factor]

            #     pu[user, factor] += lr * (err * qi_pred - reg * pu_pred)
            #     qi[item, factor] += lr * (err * pu_pred - reg * qi_pred)
