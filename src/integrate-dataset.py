...
testcase_file = "../data/testcases.txt"
...
def generate_trials(condition, diagnosed, id):#contition is the condition as a dictionary, diagnosed is a str yy-mm-dd date
    ...
    for _ in range(randint(0, maxtrials)):
        ...
        # print(start,end, is_older_then(start,end))
        while is_older_then(end, start):
            end = random_date_lim(app[0])
            # print(start,end, is_older_then(start,end))
...
def testcase_test(patid: int, condid: int):
    if(len(testcasespat) < maxtestcases):
        if(patid not in testcasespat):
            testcasespat.append(patid)
            testcasescond.append(condid)
...
def main():
    ...
    for patid in range(num_records):
        ...
        #     cured
        # }
        for _ in range(randint(1, maxcond)): #genera numero di condizioni random con limite superiore
            ...
            if cond not in curcond:
                ...
                cured = has_been_cured(trials)

                if cured:
                    cond["cured"] = trials[len(trials)-1]["end"]
                else:
                    testcase_test(int(patid), int(cond["id"]))
                    cond["cured"] = "NULL"
        ...
        patients.append( {"id": str(patid), "name": choice(n) +" "+ choice(ln), "conditions": curcond, "trials": curth} )
